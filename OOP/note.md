...
#  类的成员描述符（属性）
   ...
   - 如果想使用类的成员描述符，有三种方法
        ...
        - 使用属性修饰符
        - 使用property
            - property(fget,fset,fdel,doc)  前三个参数是函数，案例见12_属性.py
        - 无论哪种修饰符都是为了对成员属性进行相应的控制，区别在于
            - 类的方式：适合用于多个类中的多个属性共用一个描述符
...
# 7、类的常用魔术方法 见13_魔术方法.py
- 魔术方法就是不需要人为调用的方法，基本是在特定的时候自动触发
- 魔术方法有一个统一的特征：方法名被前后两个下划线包裹
    - __init__ 构造函数
    - __new__ 对象实例化方法，第一个被调用的方法，此函数比较特殊，一般不需要使用
    - __call__ 对象作为函数使用的时候触发
    - __str__ 对象作为字符串调用时候触发，python3中推荐使用
    - __repr__ 返回字符串，与__str__有一点区别，后续百度
- 描述符相关
    - __set__
    - __get__
    - __delete__
...
- 抽象类的使用：
    - 抽象类里面可以包含抽象方法，也可以包含正常的方法
    - 抽象类中可以有方法也可以有属性
    - 抽象类不能直接实例化，因为方法都没有实现
    - 必须继承才可以使用，并且继承的子类必须实现抽象类的所有抽象方法
    - 假定子类没有实现所有继承的抽象方法，则子类也不能被实例化
    - 抽象类的主要作用是设定类的标准，以便于开发的时候具有统一的规范
    
# 10、自定义类 见16_自定义类.py
- 类其实是一个类定义和方法的自由组合
- 函数名可以赋值给变量，赋值之后，此变量指向函数的内存地址，可以直接调用这个变量来调用函数
- 可以定义类和函数，借助MethodType实现
- 用type来创建一个类
- 可以利用元类实现：Metaclass